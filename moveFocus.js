...
const Flashspot = new Lang.Class({
  ...
  _init: function(area, windowMeta) {
    ...
    let constraint = Clutter.BindConstraint.new(
      this.pactor,
    ...
    this.actor.background_color = new Clutter.Color({
      red: 255,
      green: 255,
      blue: 255,
      alpha: 128,
...
MoveFocus.prototype = {
  ...
  _init: function(utils, screens) {
    ...
    this._settingsChangedListener = {
      ...
      fn: Lang.bind(this, function() {
        let enabled = this._utils.getBoolean(
          this._utils.MOVE_FOCUS_ENABLED,
          false,
        );
        if (enabled) {
          this._enable();
    ...
    this._animationSettingListener = {
      ...
      fn: Lang.bind(this, function() {
        this._animationEnabled = this._utils.getBoolean(
          this._utils.MOVE_FOCUS_ANIMATION,
          true,
        );
      }),
    };
  ...
  _enable: function() {
    ...
    this._addKeyBinding(
      'move-focus-left-screen',
      Lang.bind(this, function() {
        this._moveFocusToScreen('l');
      }),
    );
    this._addKeyBinding(
      'move-focus-right-screen',
      Lang.bind(this, function() {
  ...
  _isCandidate: function(focusWin, candidateWin, direction) {
    ...
    switch (direction) {
      case 'n':
        ...
        if (
          Math.abs(focus.y - candidate.y) + this._angle_correction <
      ...
      case 'e':
        ...
        if (
          focusRect.x + focusRect.width + this._distance_correction >=
        ...
        if (
          ...
          Math.abs(focus.x - candidate.x) + this._angle_correction
        ) {
          return false;
        }
        return true;
      case 's':
        ...
        if (
          focusRect.y + focusRect.height + this._distance_correction >=
  ...
  _cycle: function() {
    ...
    if (this._isVersion14) {
      allWin = display.sort_windows_by_stacking(
        display.get_tab_list(
  ...
  _getCurrentScreenIndex: function(win) {
    if (this._utils.getScreen().get_monitor_index_for_rect) {
      return this._utils
        .getScreen()
        .get_monitor_index_for_rect(win.get_frame_rect());
    }
  ...
  _getWindowList: function() {
    let display = global.display;
    if (this._isVersion14) {
      return display.get_tab_list(
        Meta.TabList.NORMAL_ALL,
        this._utils.getWorkspaceManager().get_active_workspace(),
      );
    } else {
      return display.get_tab_list(
        Meta.TabList.NORMAL_ALL,
        this._utils.getScreen(),
        this._utils.getWorkspaceManager().get_active_workspace(),
      );
    }
  },
  ...
  _focusNearestCandidate: function(candidates) {
    ...
    candidates.sort(function(a, b) {
      ...
      return Math.abs(a.dist - b.dist) <= distance_correction
        ? -1
        : a.dist - b.dist;
    });
